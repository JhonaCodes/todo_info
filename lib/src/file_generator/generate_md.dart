...
class MDGenerator extends GenerateBase{
  ...
  String exportTextFile(){
    ...
    report.writeln('| ***File*** | ***Líne*** | ***Priority*** | ***Level*** |***Descriptión*** |');
    report.writeln('|:---|:---:|:---|:---:|:---|');

    reportList.sort((a,b)=> b.level.compareTo(a.level));

    for (var reportData in reportList) {
      report.writeln('| [${reportData.filePath}](${reportData.filePath}) | ${reportData.lineLocation} | ${reportData.priority[0].toUpperCase()}${reportData.priority.substring(1)} | ${reportData.level} | ${reportData.description}');
    }
